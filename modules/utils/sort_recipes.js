...
let recipesFilePath = '';
let sortedRecipesFilePath = '';

try {
    console.log(path.join(__dirname, 'data', 'recipes.json'));
	recipesFilePath = path.join(__dirname, 'data', 'recipes.json');
} catch (error) {
	console.log(recipesFilePath);
	console.error(error);
}
try {
	sortedRecipesFilePath = path.join(__dirname, 'data', 'sorted_recipes.json');
} catch (error) {
	console.log(sortedRecipesFilePath);
	console.error(error);
}
...
function sortRecipes() {
	fs.readFile(recipesFilePath, (err, data) => {
		...
		for (let breakfastRecipe in breakfastRecipes) {
			...
			sortedJSONFileOBJ[breakfastRecipeName] = {
				info: {
			...
			for (let lunchRecipe in lunchRecipes) {
				...
				sortedJSONFileOBJ[breakfastRecipeName][lunchRecipeName] = {
					info: {
						id: lunchRecipeID,
						...
						ratings: recipesJSON[lunchRecipeID].ratings,
				...
				for (let dinnerRecipe in dinnerRecipes) {
					...
					sortedJSONFileOBJ[breakfastRecipeName][lunchRecipeName][dinnerRecipeName] = {
						info: {
							id: dinnerRecipeID,
							...
							ratings: recipesJSON[dinnerRecipeID].ratings,
		...
		fs.writeFile(sortedRecipesFilePath, jsonData, (err) => {
			if (err) {
				console.error(err);
			}
		});
